...
// Retrieve the earthquake GeoJSON data for allEarthquakes
...
// Gather the majorEarthquake Data
...
d3.json(majorEarthquakeData).then(function(data) {
	...
	// 6. Use the function that determines the radius of the earthquake marker baed on its magnitude
	function getRadius(magnitude) {
		if (magnitude === 0) {
			return 1;
		}
...
// Fault Line Boundries originally created by Hugo Ahlenius, Nordpil and Peter Bird
// Licensed under the Open Data Commons Attribution License
// GeoJSON format was created in 2014 by csterling 
let faultLineData = "https://raw.githubusercontent.com/fraxen/tectonicplates/master/GeoJSON/PB2002_boundaries.json";

// set the line color and weight for the fault lines
let lineStyle = {
  color: "#ff7438",
  weight: 2
}
...
d3.json(faultLineData).then(function(data) {
  console.log(data);
  // Create a GeoJSON Layer showing the fault lines
  L.geoJSON(data, {
    style: lineStyle
